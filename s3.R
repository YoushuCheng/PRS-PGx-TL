...
best = read.table(paste0(s2_output, '_initial_',initial,'_',best_criterion,'.txt'),header=T)
best_lambda = best[1,'lambda']
best_factor = best[1,'factor']
best_step = best[1,'steps']
obj = adjust_y(ped)
GD_obj = GD(obj, G, bim_sum_stats, 30, num_snp, best_lambda, best_factor)
...
if (nrow(geno) > 0) {
    ...
    table(colnames(geno) == rownames(oc_g))
